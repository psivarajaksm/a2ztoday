...
public class DeductionAllReport {
    ...
    public void DeductionAllGrandTotal(PaySlipModel psm, String filePath) {
        try {
            PrintWriter pw = null;
            File file = new File(filePath + "/DeductionAll.txt");
            try {
...
//        System.out.println("******************* PLIScheduleReport class getDeductionAllPrintWriter method is calling ***************");
        ...
        try {
            ...
            pw.println();
            count++;
            pw.printf("%3s%s%-10s%s%-30s%s%-13s%s%9s%s%-9s", psm.getSlipno(), " ", psm.getPfno(), " ", psm.getEmployeename(), " ", psm.getDesignation(), " ", psm.getAmount(), " ", psm.getInstallment());
            pw.println();
