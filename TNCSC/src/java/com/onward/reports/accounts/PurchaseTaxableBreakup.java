...
public class PurchaseTaxableBreakup implements PurchaseInterface{
    ...
    public void Header(AccountsModel am, PrintWriter pw) {
        ...
        pw.println();
        if (am.getBreakuptype().equals("3")) {
            pw.printf("%-63s%-5s","TAXABLE GOODS PURCHASED (TAX BREAKUP DETAILS) DURING THE MONTH ",am.getAccountingmonthandyear());
        } else if (am.getBreakuptype().equals("4")) {
            pw.printf("%-67s%-5s","NON-TAXABLE GOODS PURCHASED (TAX BREAKUP DETAILS) DURING THE MONTH ",am.getAccountingmonthandyear());
        }
        pw.println();
