...
public class EPFformReport {
    ...
    private double pageper833 = 0;
    private double pageper367 = 0;
    ...
    private double grper833 = 0;
    private double grper367 = 0;
    ...
    public void EPFformgrandtotal(PrintWriter pw, PaySlipModel psm) {
        ...
        pw.println();
        pw.printf("%9s%3s%s%-44s%8s%8s%8s%8s%8s%8s%7s%7s%8s", "Page No.", pgno, " ", "PAGE TOTAL", Math.round(pagesalary), Math.round(pageepf), Math.round(pageloan), Math.round(pagevpf), "0", Math.round(pageemptotal), Math.round(pageper833), Math.round(pageper367), Math.round(pageremarkstotal));
        pagesalary = 0;
        pageepf = 0;
        pageloan = 0;
        pagevpf = 0;
        ...
        pw.print(equalline);
        pw.println();
        pw.printf("%13s%-44s%8s%8s%8s%8s%8s%8s%7s%7s%8s", "", "GRAND TOTAL", Math.round(grsalary), Math.round(grepf), Math.round(grloan), Math.round(grvpf), "0", Math.round(gremptotal), Math.round(grper833), Math.round(grper367), Math.round(grremarkstotal));
        pw.println();
        pw.print(equalline);
        pw.println();
        pw.write(FORM_FEED);
    ...
    public void getEPFformPrintWriter(PaySlipModel psm, String filePath) {
        ...
        try {
            ...
            pw.printf("%3s%s%-10s%s%-30s%s%-10s%s%8s%s%7s%s%7s%s%7s%s%7s%s%7s%s%6s%s%6s%s%7s", psm.getSlipno(), " ", psm.getPfno(), " ", psm.getEmployeename(), " ", psm.getDesignation(), " ", psm.getSalary(), " ", psm.getEpf(), " ", psm.getEpfloan(), " ", psm.getVpf(), " ", "0", " ", psm.getEmployertotal(), " ", psm.getPercentage833(), " ", psm.getPercentage367(), " ", psm.getRemarkstotal());
