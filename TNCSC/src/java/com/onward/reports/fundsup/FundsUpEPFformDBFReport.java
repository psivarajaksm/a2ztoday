...
public class FundsUpEPFformDBFReport {
    ...
    public FundsUpEPFformDBFReport() {
        for (int i = 0; i < 140; i++) {
            horizontalsign[i] = '~';
        }
        for (int i = 0; i < 140; i++) {
...
//                pw.printf("%-3s%-3s%s%-2s%-10s%6s%5s%5s%5s%5s%4s%5s%5s%5s%2s%s%2s", psm.getBranch(), psm.getPayslipmonth(), "\"", psm.getPayslipyear().substring(2, 4), psm.getPfno(), psm.getSalary(), psm.getEpf(), "0", psm.getEpfloan(), psm.getVpf(), "0", psm.getPercentage367(), psm.getPercentage833(), "0", psm.getEmployeetype(), psm.getSalarytype(), (psm.getSupplementarytype() != null) ? psm.getSupplementarytype() : "");  
                pw.printf("%-30s", psm.getBranch() + "," + psm.getPayslipmonth() + "\"" + psm.getPayslipyear().substring(2, 4) + "," + psm.getPfno() + "," + psm.getSalary() + "," + psm.getEpf() + ",0," + psm.getEpfloan() + "," + psm.getVpf() + "," + psm.getDavpf() + "," + psm.getPercentage367() + "," + psm.getPercentage833() + ",0,"+psm.getEmployeetype()+","+ psm.getSalarytype()+","+  psm.getSupplementarytype());
            pw.println();
            pw.flush();
            pw.close();
