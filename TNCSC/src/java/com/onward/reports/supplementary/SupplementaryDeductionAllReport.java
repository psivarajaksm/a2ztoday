...
public class SupplementaryDeductionAllReport {
    ...
    public SupplementaryDeductionAllReport() {
        for (int i = 0; i < 80; i++) {
            horizontalsign[i] = '~';
        }
        for (int i = 0; i < 80; i++) {
...
//        System.out.println("******************* PLIScheduleReport class getDeductionAllPrintWriter method is calling ***************");
        ...
        try {
            ...
            pw.println();
            count++;
            pw.printf("%3s%s%-10s%s%-30s%s%-13s%s%9s%s%-9s", psm.getSlipno(), " ", psm.getPfno(), " ", psm.getEmployeename(), " ", (psm.getDesignation() == null) ? "" : psm.getDesignation(), " ", psm.getAmount(), " ", psm.getInstallment());
            pw.println();
