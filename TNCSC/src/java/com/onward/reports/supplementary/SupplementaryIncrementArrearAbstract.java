...
public class SupplementaryIncrementArrearAbstract {
    ...
    public SupplementaryIncrementArrearAbstract() {
        for (int i = 0; i < 80; i++) {
            horizontalsign[i] = '~';
        }
        for (int i = 0; i < 80; i++) {
...
//        System.out.println("***************** SupplementaryAbstractReport class getSalaryAbstractPrintWriter method is calling *******************");
        ...
        try {
            ...
            pw.println();
            pw.printf("%60s", "TAMIL NADU CIVIL SUPPLIES CORPORATION");
            pw.println();
            pw.printf("%55s", "INCREMENT ARREAR ABSTRACT");
            pw.println();
            pw.printf("%-9s%-58s%-10s", "REGION : ", psm.getBranch(), psm.getPayslipyear());
            pw.println();
            pw.print(horizontalsign);
            pw.println();
            pw.printf("%s", "       ACCOUNT HEAD          EARNINGS      ACCOUNT HEAD        DEDUCTIONS");
            pw.println();
            pw.print(horizontalsign);
            pw.println();
            ...
            pw.write(equalline);
            pw.println();
            pw.printf("%44s%-10s%8s%11s", "NET AMOUNT PAYABLE FOR INCREMENT ARREAR ON ", psm.getPayslipyear(), "RS.  ", psm.getNetsalary());
